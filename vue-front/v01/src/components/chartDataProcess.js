...
export function makeOnlineDataset(rawdata) {
...
export function makeCountDataset(rawdata) {
    let dataset = []
    // 构建第一行表头
    let head = ['time', 'all']
...
// 做series
export function makeCountSeries() {
    // 做一个需要数据的字段列表
    let l = ['all']
    for (var i in _REGION_NAME) {
        l.push(i)
...
export let makeActivitySeries = makeCountSeries
// 生成饼图需要的数据，使用做好的count的dataset
// 生成[{name:'xxx',value:'yyy'},{}]
export function makePieData(dataset, idx) {
    let head = dataset[0]
    //第一行是表头，+1跳过
...
// ]
//
export function makeActivityDataset(rawdata) {
    if (rawdata.length < 2) {
        return []
    }
